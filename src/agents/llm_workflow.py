...
def parse_resume_with_deepseek(
    text: str,
    client: DeepSeekClient,
    lang: str = DEFAULT_LANG,
) -> tuple[MasterResume, TokenLog | None, str | None]:
    ...
    return _parse_or_fallback(result.data, MasterResume, result.token_log, lambda: parse_resume_text_fallback(text, lang), lang)


def analyze_jd_with_deepseek(
    text: str,
    client: DeepSeekClient,
    lang: str = DEFAULT_LANG,
) -> tuple[JDAnalysis, TokenLog | None, str | None]:
    ...
    return _parse_or_fallback(result.data, JDAnalysis, result.token_log, lambda: analyze_jd_fallback(text, lang), lang)
...
def extract_fact_with_deepseek(
    ...
    *,
    ...
    return _parse_or_fallback(result.data, FactCard, result.token_log, fallback, lang)
...
def _parse_or_fallback(
    ...
    try:
        if data is None:
            raise ValueError(t("llm.empty_json", lang))
        return schema.model_validate(data), token_log, None
    except (ValidationError, ValueError, TypeError) as exc:
        return fallback(), token_log, t("llm.fallback_warning", lang, error=exc)
